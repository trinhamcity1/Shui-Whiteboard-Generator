...
**Amended after this phase shipped and was tested — see
`shui-wg-phase-04-amendment-asset-library.md`.** That amendment supersedes the
"live generation every render" approach described below with a reusable asset
library, after a direct comparison against Golpo's actual output. Everything below
about the `ImageProvider` interface, both providers, caching, and cost telemetry
stays true and stays built; only the *default* generation timing/strategy changed.
Read this file first for the foundational architecture, then read the amendment.
...
- **Flux Schnell** — fast raster diffusion, via an aggregator (fal.ai or Replicate).
  Cheaper per image (roughly $0.015–0.02/image via fal.ai), photorealistic or painterly
...
**Update: character recurrence across videos turned out to matter far more than this
document assumed — see the amendment.**
