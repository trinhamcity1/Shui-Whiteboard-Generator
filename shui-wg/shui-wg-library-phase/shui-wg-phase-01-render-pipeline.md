...
Phase 4 itself was later amended — see `shui-wg-phase-04-amendment-asset-library.md` —
to source most character/prop illustration from a pre-built reusable asset registry
instead of generating live per render. `DocumentReveal` and `FullBleedGraphic` are
unaffected: they still just take a URL, whether that URL comes from a live API call or
a registry lookup. A new `SketchDiagram` component (using `rough.js`) was added
alongside them for structured diagrams with real, always-correctly-spelled text.
