...
later, demand-driven decision, not something to over-build speculatively now).

---

## Amendment (added when Phase 4 was written)

`planScenesFromScript`, as built in this phase, was deliberately restricted to the
purely typographic component types — its own system prompt explicitly forbade
`documentReveal` and `fullBleedGraphic`, since at the time nothing in the pipeline
could produce a real image for them. That restriction is lifted in
`shui-wg-phase-04-illustration-engine.md`: the planner may now request an illustration
by describing it (`imageConcept`), and a new pipeline step resolves that into a real
generated image before render. The icon vocabulary and style-variant theming built here
are unaffected and are reused as-is by the new illustration prompts (each style variant
now also shapes how illustration requests are phrased to the image provider).

## Amendment (added when Phase 4 was further amended)

Phase 4 was later amended — see `shui-wg-phase-04-amendment-asset-library.md` — so that
most character/prop `imageConcept` requests resolve against a reusable asset registry
(built once) rather than a live provider call. The style-variant theming built here
still governs the *look* of everything on screen — the asset library's illustration
style was itself derived from a close comparison against Golpo's reference style, and
should visually match the `classic-whiteboard` theme's palette and tone.
