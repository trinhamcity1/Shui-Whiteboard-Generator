...
// ~$0.0000025/GiB-second. Phase 0 runs locally, so this is a projection of
...
export function buildJobCost(args: {
  ...
  return {
    ...
    totalCostUsd: args.ttsCostUsd + renderComputeCostUsd,
  };
}

export function printJobCost(cost: JobCost): void {
  ...
  if (!withinTarget) {
    console.warn(
      "\n⚠️  WARNING: total cost is well outside the $0.05-$0.20/minute target this project is betting on. Flag this before continuing to Phase 1.",
    );
  }
  console.log("---------------------------\n");
}
